...
def test_packet_count():
    ...
    def receiving_app():
        ...
        # Handed to call_from_thread above, so it is never invoked here - but
        # the attribute still has to exist to be passed. See
        # test_every_packet_reaches_the_table for the row it produces.
        app._add_packet_row = lambda pkt: None
        return app
...
def test_packet_table_row_text():
    print("a text packet becomes one table row")
    row = bot.packet_table_row(
        _text_packet("ping", bot.BROADCAST_ADDR, 601),
        {"!them": {"user": {"shortName": "Bug2"}}},
        "!me",
    )
    check("type is TEXT", row["type"], "TEXT")
    check("the sender name resolves", row["node"], "Bug2")
    check("channel is carried", row["channel"], 3)
    check("snr is carried", row["snr"], 6.5)
    check("rssi is carried", row["rssi"], -92)
    check("detail holds the message text", row["detail"], "ping")


def _telemetry_packet(channel=0, from_id="!them"):
    """One TELEMETRY_APP packet, shaped the way the library delivers them."""
    return {
        "decoded": {
...
def test_packet_table_row_telemetry():
    print("a telemetry packet reports voltage and battery, not empty text")
    row = bot.packet_table_row(
        _telemetry_packet(),
        {"!them": {"user": {"shortName": "bug3"}}},
        "!me",
    )
    check("type says TELEMETRY", row["type"], "TELEMETRY")
    check("detail carries the metrics", row["detail"], "4.32V 101%")


def test_packet_table_row_encrypted():
    print("a packet that never decoded is labelled, not left blank")
    row = bot.packet_table_row(
        {
            "from": 0xB2A70EAC,
            "fromId": "!b2a70eac",
    ...
    # A blank type column would be indistinguishable from a rendering bug; the
    # mesh carries these constantly and they are meaningful as themselves.
    check("type says encrypted", row["type"], "encrypted")
    check("node falls back to the id", row["node"], "!b2a70eac")
    check("detail stays empty", row["detail"], "")
    check("a missing snr is None, not 0", row["snr"], None)


def test_packet_table_row_hops():
    print("hops are reported as travelled out of the starting limit")
    packet = _text_packet("hi", bot.BROADCAST_ADDR, 602)
    packet["hopStart"] = 3
    packet["hopLimit"] = 1
    check("two of three hops used", bot.packet_table_row(packet, {}, "!me")["hops"], "2/3")

    # Direct neighbours arrive with the limit untouched.
    packet["hopLimit"] = 3
    check("an untravelled packet reads 0", bot.packet_table_row(packet, {}, "!me")["hops"], "0/3")

    # The library omits both keys on some paths, and a made-up "0/0" would read
    # as a fact rather than an absence.
    bare = _text_packet("hi", bot.BROADCAST_ADDR, 603)
    check("absent hop fields read as --", bot.packet_table_row(bare, {}, "!me")["hops"], "--")


def test_packet_table_row_when():
    print("the time column prefers the node's clock over ours")
    packet = _text_packet("hi", bot.BROADCAST_ADDR, 604)
    packet["rxTime"] = 1788706684
    check(
        "a reported time is used as-is",
        bot.packet_table_row(packet, {}, "!me")["when"],
        _dt.datetime.fromtimestamp(1788706684).strftime("%H:%M:%S"),
    )

    # A node that has never had a GPS fix or a phone attached reports 0, which
    # is the normal state for a bench node. Falling back to our own clock is
    # honest - the packet is rendered as it arrives - and "~" says it was
    # derived here, the same convention parse_incoming uses.
    packet["rxTime"] = 0
    check(
        "a derived time is marked",
        bot.packet_table_row(packet, {}, "!me")["when"].startswith("~"),
        True,
    )


def test_packet_table_row_nodeinfo():
    print("a nodeinfo packet names the node it describes")
    packet = {
        "decoded": {
            "portnum": "NODEINFO_APP",
            "user": {"shortName": "Bug2", "longName": "BUG2 Router"},
        },
        "from": 0xF2DCBABE,
        "fromId": "!them",
        "channel": 0,
    }
    row = bot.packet_table_row(packet, {}, "!me")
    check("type says NODEINFO", row["type"], "NODEINFO")
    check("detail carries the long name", row["detail"], "BUG2 Router")


def test_packet_table_row_traceroute():
    print("a traceroute packet is named rather than shown as a raw portnum")
    packet = {
    ...
    check("type says TRACEROUTE", bot.packet_table_row(packet, {}, "!me")["type"], "TRACEROUTE")

    # An unmapped port number is shown as the library named it, so firmware
    # that adds one stays legible instead of rendering blank.
    packet["decoded"]["portnum"] = "RANGE_TEST_APP"
    check("an unmapped type passes through", bot.packet_table_row(packet, {}, "!me")["type"], "RANGE_TEST_APP")


async def _packet_table_widget():
    from textual.widgets import DataTable

    app = bot.MeshtasticTUI()
    async with app.run_test():
        table = app.query_one("#packet-table", DataTable)
        check("the packet table starts empty", table.row_count, 0)

        # Every packet reaches the table, including the types parse_incoming
        # drops - that is the whole point of the pane.
        app._add_packet_row(_telemetry_packet())
        check("a telemetry packet lands as a row", table.row_count, 1)
        app._add_packet_row(_text_packet("ping", bot.BROADCAST_ADDR, 701))
        check("a text packet lands too", table.row_count, 2)

        rendered = " ".join(str(c) for c in table.get_row_at(0))
        check("the row carries the type", "TELEMETRY" in rendered, True)
        check("and the metrics summary", "4.32V 101%" in rendered, True)


def test_packet_table_widget():
    print("packets reach a real DataTable in the status pane")
    # Headless, so it still needs no hardware. A stand-in self cannot exercise
    # this: DataTable.add_row is what turns a dict into a visible row.
    asyncio.run(_packet_table_widget())


async def _packet_table_is_capped():
    from textual.widgets import DataTable

    app = bot.MeshtasticTUI()
    async with app.run_test():
        table = app.query_one("#packet-table", DataTable)
        for n in range(bot.PACKET_TABLE_LIMIT + 25):
            app._add_packet_row(_text_packet(f"m{n}", bot.BROADCAST_ADDR, 800 + n))
        # This pane runs for days on a mesh moving hundreds of packets a
        # minute, so an uncapped table is a slow leak.
        check("the table stops growing at the cap", table.row_count, bot.PACKET_TABLE_LIMIT)


def test_packet_table_is_capped():
    print("the packet table discards its oldest rows instead of growing forever")
    asyncio.run(_packet_table_is_capped())


def test_every_packet_reaches_the_table():
    print("the packet table is fed ahead of the text-message filter")
    fed = []
    app = types.SimpleNamespace(
        packet_count=0, received_count=0, last_signal=None,
        my_id="!me", history={}, target=None, last_packet_at=None,
    ...
    app._log_system = lambda line: None
    app._should_auto_reply = lambda info: False
    app._add_packet_row = fed.append
    # Run the handed-off work inline; the point here is *what* gets scheduled.
    app.call_from_thread = lambda fn, *a: fn(*a)

    interface = types.SimpleNamespace(nodes={})
    # parse_incoming returns None for a telemetry packet, so reaching the table
    # at all proves the table is fed before that filter runs.
    bot.MeshtasticTUI.on_receive(app, _telemetry_packet(), interface)
    check("a packet parse_incoming drops still reaches the table", len(fed), 1)

    # And a text message is not fed twice just because it also has a message row.
    bot.MeshtasticTUI.on_receive(app, _text_packet("ping", bot.BROADCAST_ADDR, 901), interface)
    check("a text packet is fed exactly once", len(fed), 2)
...
if __name__ == "__main__":
    ...
    try:
        ...
        test_distance_to()
        test_reply_text()
        test_packet_table_row_text()
        test_packet_table_row_telemetry()
        test_packet_table_row_encrypted()
        test_packet_table_row_hops()
        test_packet_table_row_when()
        test_packet_table_row_nodeinfo()
        test_packet_table_row_traceroute()
        test_packet_table_widget()
        test_packet_table_is_capped()
        test_every_packet_reaches_the_table()
        test_packet_node_id()
        test_format_plain()
