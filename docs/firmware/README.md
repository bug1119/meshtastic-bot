...
中文字型現在也備份了,但備份的是**配方而不是產物** —— 見 [`fonts/`](fonts/)。`lv_font_conv` 把它收到的參數寫在產物檔頭裡,所以 30 KB 的字碼表加幾個參數就能重建那 16.4 MB。原始字型 Noto Sans TC 是 Google 的公開字型,從來就不會遺失。
...
- ~~中文字型沒有機器外備份~~ **已解決,而且我原本的判斷是錯的。** 我以為「產生流程沒有記錄」,但 `lv_font_conv` 會把收到的參數寫進產物檔頭 —— **配方一直都在那 16 MB 裡面**,只是沒人去讀。現在存成 [`fonts/`](fonts/):30 KB 取代 16.4 MB,小 567 倍,而且能完整重建。
