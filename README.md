...
201 項檢查,不需要 pytest 也不需要硬體(但因為它直接 `import bot`,所以仍要那三個套件 —— shebang 已經處理好了)。涵蓋規則解析與優先序、連線時的規則覆蓋回報、裝置 key 與 host:port 解析、中文顯示寬度、位置擷取與距離、頻率/頻寬推導、未讀粗體、斷線偵測與重連退避,以及自動回覆的文字組成。
